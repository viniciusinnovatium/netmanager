...
public class COMLogin extends mPage {
	...
	public Object OnPage(Object... _p) {
		...
		// << ; 24-Nov-2010 shobby SR17247: Allow changing the welcome message
		// at a company
		// << ; level.
		// << ; 03-May-2010 shobby SR17253 (1.7.1) Invalid syntax
		// << ; 16-Oct-2009 shobby SR16948: Can't use $job as a subscript to
		// preserve
		// << ; URL in CacheTempURL. Passed in to variable on
		// << ; the form.
		// << ; 02-Dec-2008 FIS SR16205: Simple Login screen for PDA's
		// << ; 09-Sep-2008 shobby BR014979: Reverted on-site change to hardcode
		// login
		// << ; screen to Portuguese
		// << ; 11-Sep-2007 shobby BR014713: If invalid company passed as
		// parameter
		// << ; reset to company 0
		// << ; 09-Aug-2007 shobby BR014237: Store the calling URL so that it
		// can be
		...
		// << ; 15-Feb-2006 shobby BR014393: shobby... Merged changes from
		// Gustavo F
		// << ; 02-Nov-2006 shobby BR014210: Store YM in a hidden field in
		// passed as a
		// << ; parameter from the URL.
		// << ; 31-Oct-2006 GM BR014210: Call ^WWWVAR as first step, before
		// "set YM"
		// << ; 18-Oct-2006 shobby BR014234: Allow passing in of language from
		// the URL
		// << ; to enable language translation of the welcome.
		// << ; 01-Feb-2006 PO SR14047: Keep track of "sessions"
		// << ; 24-Oct-2005 shobby SR12338: Code changes for colours and logo
		// pictures as required in Brazil.
		// <<
		// ;------------------------------------------------------------------------------
		// << new
		// bkgColor,blnSessionLoginOK,ColorCodeForHeaderLeft,ImageDirectory,logoPicture
		...
		m$.newVar(bkgColor, blnSessionLoginOK, ColorCodeForHeaderLeft,
				ImageDirectory, logoPicture);
		// << new
		// objWWW012,SPRACHE,strLanguage,strLoginError,strLoop,strURLParameters,YM
		...
		mVar YM = m$.var("YM");
		m$.newVar(objWWW012, SPRACHE, strLanguage, strLoginError, strLoop,
				strURLParameters, YM);
		// <<
		...
		// << if ($get(%request) '= "") &&
		// $find($get(%request.CgiEnvs("HTTP_USER_AGENT")),"Windows CE") {
		if ((mOp.NotEqual(m$.Fnc.$get(m$.getRequest()), ""))
				&& mOp.Logical(m$.Fnc.$find(
						m$.Fnc.$get(m$.getRequest().getCgiEnvs(
								"HTTP_USER_AGENT")), "Windows CE"))) {
			// << set blnIsMobile = $$$YES
		...
		// << ;--------------------------------------
		// << ; Company
		// << ; - If no company nominated or the company doesn't exist, then
		// uses the default zero company.
		...
		for (; true;) {
			...
			// <<
			// << if strURLParameters'="" set strURLParameters =
			// strURLParameters_"&"
			if (mOp.NotEqual(strURLParameters.get(), "")) {
				strURLParameters.set(mOp.Concat(strURLParameters.get(), "&"));
			}
			// << set strURLParameters =
			// strURLParameters_strLoop_"="_%request.Data(strLoop,1)
			strURLParameters.set(mOp.Concat(mOp.Concat(
					mOp.Concat(strURLParameters.get(), strLoop.get()), "="), m$
					.getRequest().getData(strLoop.get(), 1)));
		}
		// << }
		// << if strURLParameters'="" set strURLParameters =
		// %request.URL_"?"_strURLParameters
		if (mOp.NotEqual(strURLParameters.get(), "")) {
			strURLParameters.set(mOp.Concat(
					mOp.Concat(m$.getRequest().getURL(), "?"),
					strURLParameters.get()));
		}
		// <<
		// << ;--------------------------------------
		// << ; Colours
		// << ;--------------------------------------
		// << set ColorCodeForHeaderLeft =
		// $$$WWW012ColorCodeForHeaderLeft(objWWW012)
		ColorCodeForHeaderLeft.set(include.WWWConst
				.$$$WWW012ColorCodeForHeaderLeft(m$, objWWW012));
		// <<
		// << set bkgColor = ""
		bkgColor.set("");
		// << set:ColorCodeForHeaderLeft'="" bkgColor =
		// $piece($get(^WWW100(0,"FARBE","EN",ColorCodeForHeaderLeft,1)),"~",1)
		if (mOp.NotEqual(ColorCodeForHeaderLeft.get(), "")) {
			bkgColor.set(m$.Fnc.$piece(m$.Fnc.$get(m$.var("^WWW100", 0,
					"FARBE", "EN", ColorCodeForHeaderLeft.get(), 1)), "~", 1));
		}
		...
		// <<
		// << set ImageDirectory =
		// $translate($$$WWW012PictureDirectorySystem(objWWW012),"\","/") // New
		// line for service
		ImageDirectory
				.set(m$.Fnc.$translate(include.WWWConst
						.$$$WWW012PictureDirectorySystem(m$, objWWW012), "\\",
						"/"));
		// <<
		// << ; %session.Language
		// << if SPRACHE="" set SPRACHE =
		// $zconvert($piece(%request.GetCgiEnv("HTTP_ACCEPT_LANGUAGE","EN"),"-",1),"U")
		if (mOp.Equal(SPRACHE.get(), "")) {
			SPRACHE.set(m$.Fnc.$zconvert(
					m$.Fnc.$piece(
							m$.getRequest().getCgiEnv("HTTP_ACCEPT_LANGUAGE",
									"EN"), "-", 1), "U"));
		}
		// <<
		// << set strLoginError = $$$Text("Com00240",,SPRACHE)
		strLoginError
				.set(include.COMSYS.$$$Text(m$, "Com00240", null, SPRACHE));
		// <<
		// << write "<html>",!
		m$.Cmd.Write("<html>", "\n");
		// << write "<head>",!
		...
		// << write
		// " background-image: url("_ImageDirectory_$$LoginImage^WWW012()_");",!
		// ;SR18106
		m$.Cmd.Write(
				mOp.Concat(mOp.Concat(
						mOp.Concat(" background-image: url(",
								ImageDirectory.get()),
						m$.fnc$("WWW012.LoginImage")), ");"), "\n");
		...
		if (mOp.NotEqual(include.WWWConst.$$$WWW012BrowserTitle(m$, objWWW012),
				"")) {
			// << write "<title>"_$$$WWW012BrowserTitle(objWWW012)_"</title>",!
			// ;SR18073
			m$.Cmd.Write(mOp.Concat(mOp.Concat("<title>",
					include.WWWConst.$$$WWW012BrowserTitle(m$, objWWW012)),
					"</title>"), "\n");
		}
		// << } else { ;SR18073
		else {
		...
		m$.Cmd.Write("   var strCookieValue = getCookie(pstrName);", "\n");
		// << write
		// "   if (strCookieValue == null) strCookieValue = ':' + document.location.href;",!
		m$.Cmd.Write(
				"   if (strCookieValue == null) strCookieValue = ':' + document.location.href;",
				"\n");
		// << write
		// "   if (document.location.href == strCookieValue.substring(strCookieValue.indexOf(':')+1,strCookieValue.length)) {",!
		m$.Cmd.Write(
				"   if (document.location.href == strCookieValue.substring(strCookieValue.indexOf(':')+1,strCookieValue.length)) {",
				"\n");
		...
		m$.Cmd.Write("   } else {", "\n");
		// << write
		// "      document.www.action = 'javascript:alert(\'"_strLoginError_"\');';",!
		m$.Cmd.Write(mOp.Concat(mOp.Concat(
				"      document.www.action = 'javascript:alert(\\'",
				strLoginError.get()), "\\');';"), "\n");
		// << write "   }",!
		m$.Cmd.Write("   }", "\n");
		// << write "}",!
		...
		// <<
		// << if
		// (+$$$WWWClientParamCoreChangesALL($get(^WWWClientParam(YM,YM,1)))) {
		if (mOp.Logical((mOp.Positive(include.WWWConst
				.$$$WWWClientParamCoreChangesALL(
						m$,
						m$.Fnc.$get(m$.var("^WWWClientParam", YM.get(),
								YM.get(), 1))))))) {
			...
			m$.Cmd.Write("  var agent = navigator.userAgent.toLowerCase();",
					"\n");
			// << write
			// "  var isFirefox3_6 = (agent.indexOf('firefox/3.6.') != -1);", !
			m$.Cmd.Write(
					"  var isFirefox3_6 = (agent.indexOf('firefox/3.6.') != -1);",
					"\n");
			...
			// << write
			// "    document.write('    <img src="""_ImageDirectory_"/aviso_versao.png"">\n');",
			// !
			m$.Cmd.Write(
					mOp.Concat(mOp.Concat(
							"    document.write('    <img src=\"",
							ImageDirectory.get()), "/aviso_versao.png\">\\n');"),
					"\n");
		...
		// << write
		// " BGCOLOR=#C0C0C0 vlink=black link=black topmargin=1 leftmargin=1>",!
		m$.Cmd.Write(
				" BGCOLOR=#C0C0C0 vlink=black link=black topmargin=1 leftmargin=1>",
				"\n");
		// <<
		...
		if ((mOp.Equal(blnIsMobile.get(), include.COMSYS.$$$YES(m$)))) {
			// << write
			// "  <table border=1 bgcolor=white bordercolordark=gainsboro bordercolorlight=black cellspacing=0 cellpadding=4 width=100% height=100%>",!
			m$.Cmd.Write(
					"  <table border=1 bgcolor=white bordercolordark=gainsboro bordercolorlight=black cellspacing=0 cellpadding=4 width=100% height=100%>",
					"\n");
			// << write "    <tr>",!
			...
			m$.Cmd.Write("    <td>", "\n");
			// << write
			// "     <form name=""www"" action=""COMUserPreferences.cls"" METHOD=""POST"" >",!
			m$.Cmd.Write(
					"     <form name=\"www\" action=\"COMUserPreferences.cls\" METHOD=\"POST\" >",
					"\n");
			// << write
			// "       <input type=hidden name=YUCI value="""_$znspace_""">",!
			m$.Cmd.Write(mOp.Concat(mOp.Concat(
					"       <input type=hidden name=YUCI value=\"",
					m$.Fnc.$znspace()), "\">"), "\n");
			// << write
			// "       <input type=hidden name=YXURL value="""_strURLParameters_""">",!
			m$.Cmd.Write(mOp.Concat(mOp.Concat(
					"       <input type=hidden name=YXURL value=\"",
					strURLParameters.get()), "\">"), "\n");
			// <<
			// << ; SRBR014210 - Only set this if the parameter is passed in
			// from the icon.
			// << ; Not 100% confident that this will have no impact on existing
			// << ; functionality when no value is passed in from the URL.
			// << if $get(%request.Data("YM",1))'="" write
			// " <input type=hidden name=YM value="_""""_YM_""""_">",!
			if (mOp.NotEqual(m$.Fnc.$get(m$.getRequest().varData("YM", 1)), "")) {
				m$.Cmd.Write(mOp.Concat(mOp.Concat(mOp.Concat(
						mOp.Concat(" <input type=hidden name=YM value=", "\""),
						YM.get()), "\""), ">"), "\n");
			}
			// <<
			...
			m$.Cmd.Write("       <input type=hidden name=YRANDOM value=0>",
					"\n");
			// << write
			// "       <img src="_ImageDirectory_"/"_logoPicture_" border=0><br><br>"
			m$.Cmd.Write(mOp.Concat(mOp.Concat(
					mOp.Concat(mOp.Concat("       <img src=",
							ImageDirectory.get()), "/"), logoPicture.get()),
					" border=0><br><br>"));
			// << write
			// "          <span class=""userandpass"">"_$$$Text(414,,SPRACHE)_"</span>",!
			// ;Username
			m$.Cmd.Write(
					mOp.Concat(mOp.Concat(
							"          <span class=\"userandpass\">",
							include.COMSYS.$$$Text(m$, 414, null, SPRACHE)),
							"</span>"), "\n");
			// << if
			// (+$$$WWWClientParamCoreChangesALL($get(^WWWClientParam(YM,YM,1))))
			// {
			if (mOp.Logical((mOp.Positive(include.WWWConst
					.$$$WWWClientParamCoreChangesALL(
							...
							m$.Fnc.$get(m$.var("^WWWClientParam", YM.get(),
									YM.get(), 1))))))) {
				// << write
				// "          <input name="_""""_"YBED"_""""_" type=text class=""estilom3"" size=18 maxlength=30><br>",!
				m$.Cmd.Write(
						mOp.Concat(mOp.Concat(mOp.Concat(
								mOp.Concat("          <input name=", "\""),
								"YBED"), "\""),
								" type=text class=\"estilom3\" size=18 maxlength=30><br>"),
						"\n");
			}
			// << }
			// << else {
				...
				// << write
				// "          <input name="_""""_"YBED"_""""_" type=text class=""estilom3"" size=18 maxlength=20><br>",!
				m$.Cmd.Write(
						mOp.Concat(mOp.Concat(mOp.Concat(
								mOp.Concat("          <input name=", "\""),
								"YBED"), "\""),
								" type=text class=\"estilom3\" size=18 maxlength=20><br>"),
						"\n");
			}
			// << }
			// << write
			// "          <span class=""userandpass"">"_$$$Text("3",,SPRACHE)_"</span>",!
			// ; "Password"
			m$.Cmd.Write(
					mOp.Concat(mOp.Concat(
							"          <span class=\"userandpass\">",
							include.COMSYS.$$$Text(m$, "3", null, SPRACHE)),
							"</span>"), "\n");
			// << write
			// "          <input name=YPWD type=password class=""estilom3"" size=18 maxlength=""20"" maxlenth=20><br>",!
			m$.Cmd.Write(
					"          <input name=YPWD type=password class=\"estilom3\" size=18 maxlength=\"20\" maxlenth=20><br>",
					"\n");
			// << write
			// "          <input type=""submit"" name=""Submit"" value=""&nbsp;"_$$$Text("67",,SPRACHE)_"&nbsp;"">&nbsp;&nbsp;&nbsp;",!
			// ; "Start"
			m$.Cmd.Write(
					mOp.Concat(
							mOp.Concat(
									"          <input type=\"submit\" name=\"Submit\" value=\"&nbsp;",
									include.COMSYS.$$$Text(m$, "67", null,
											SPRACHE)),
							"&nbsp;\">&nbsp;&nbsp;&nbsp;"), "\n");
			// << write
			// "          <input type=""reset"" name=""New"" onClick='document.www.YBED.focus();' value=""&nbsp;"_$$$Text("58",,SPRACHE)_"&nbsp;"">",!
			// ; "New"
			m$.Cmd.Write(
					mOp.Concat(
							mOp.Concat(
									"          <input type=\"reset\" name=\"New\" onClick='document.www.YBED.focus();' value=\"&nbsp;",
									include.COMSYS.$$$Text(m$, "58", null,
											SPRACHE)), "&nbsp;\">"), "\n");
			// << write "      </td>",!
			m$.Cmd.Write("      </td>", "\n");
			// << write "    </tr>",!
		...
		// << } else {
			...
			// << ;---------------------------------------
			// << write
			// "  <table border=1 bordercolordark=gainsboro bordercolorlight=black cellspacing=0 width=100% height=100%>",!
			m$.Cmd.Write(
					"  <table border=1 bordercolordark=gainsboro bordercolorlight=black cellspacing=0 width=100% height=100%>",
					"\n");
			// << write "    <tr>",!
			m$.Cmd.Write("    <tr>", "\n");
			// << write
			// "   <td width=214 height=446 align=middle valign=top bgcolor=#FFFFFF>",!
			m$.Cmd.Write(
					"   <td width=214 height=446 align=middle valign=top bgcolor=#FFFFFF>",
					"\n");
			// << if '$$$WWW012HideLogoSpacer(objWWW012) write "         <br>",!
			// ; SR17793
			if (mOp.Not(include.WWWConst.$$$WWW012HideLogoSpacer(m$, objWWW012))) {
				m$.Cmd.Write("         <br>", "\n");
			}
			// << write
			// "         <img src="_ImageDirectory_"/"_logoPicture_" border=0>",!
			m$.Cmd.Write(mOp.Concat(mOp.Concat(
					mOp.Concat(
							mOp.Concat("         <img src=",
									ImageDirectory.get()), "/"),
					logoPicture.get()), " border=0>"), "\n");
			// << write "      </td>",!
			m$.Cmd.Write("      </td>", "\n");
			// << write
			// "   <td align=middle bgcolor="""_bkgColor_"""><br><br>",!
			m$.Cmd.Write(mOp.Concat(mOp.Concat(
					"   <td align=middle bgcolor=\"", bkgColor.get()),
					"\"><br><br>"), "\n");
			// << write
			// "     <form name=""www"" action="""_..#LOGINSUBMIT_""" METHOD=""POST"" >",!
			m$.Cmd.Write(mOp.Concat(
					mOp.Concat("     <form name=\"www\" action=\"",
							m$.param(this, "p_LOGINSUBMIT").get()),
					"\" METHOD=\"POST\" >"), "\n");
			// << write
			// "       <input type=hidden name=YUCI value="""_$znspace_""">",!
			m$.Cmd.Write(mOp.Concat(mOp.Concat(
			...
			// << ;---------------------------------------
			// << write
			// "             <table width=""100%"" border=""0"" cellpadding=""0"" cellspacing=""0"" class=""estilom1"">",!
			m$.Cmd.Write(
					"             <table width=\"100%\" border=\"0\" cellpadding=\"0\" cellspacing=\"0\" class=\"estilom1\">",
					"\n");
			// << write "             <tr>",!
			...
			// << //SR17247 write
			// "                 <td colspan=""3"" valign=""top"" class=""estilom2""><div align=""center"">"_$$$Text("413",,SPRACHE)_" "_$$$WWW012CompanyName(objWWW012)_"</div></td>",!
			// ;Welcome To ;BR014979
			// << write
			// "               <td colspan=""3"" valign=""top"" class=""estilom2""><div align=""center"">"_$$WelcomeMessage^WWW012(SPRACHE)_..#EXTRAWELCOMEMESSAGE_"</div></td>",!
			// ;Welcome To ;BR014979
			m$.Cmd.Write(
					mOp.Concat(
							mOp.Concat(
									mOp.Concat(
											"               <td colspan=\"3\" valign=\"top\" class=\"estilom2\"><div align=\"center\">",
											m$.fnc$("WWW012.WelcomeMessage",
													SPRACHE.get())),
									m$.param(this, "p_EXTRAWELCOMEMESSAGE")
											.get()), "</div></td>"), "\n");
			// << write "               <td>&nbsp;</td>",!
			m$.Cmd.Write("               <td>&nbsp;</td>", "\n");
			// << write "             </tr>",!
			...
			m$.Cmd.Write("                 <tr>", "\n");
			// << write
			// "                   <td width=""270"" height=""86"" valign=""top"">",!
			m$.Cmd.Write(
					"                   <td width=\"270\" height=\"86\" valign=\"top\">",
					"\n");
			// <<
			...
			// << ;---------------------------------------
			// << write
			// "                        <table width=""273"" height=""100"" border=""0"">",!
			m$.Cmd.Write(
					"                        <table width=\"273\" height=\"100\" border=\"0\">",
					"\n");
			// << write "                          <tr>",!
			m$.Cmd.Write("                          <tr>", "\n");
			// << write
			// "                            <td width=""61"" height=""22""><div align=""right""><span class=""userandpass"">"_$$$Text(414,,SPRACHE)_"</span></div></td>",!
			// ;Username
			m$.Cmd.Write(
					mOp.Concat(
							mOp.Concat(
									"                            <td width=\"61\" height=\"22\"><div align=\"right\"><span class=\"userandpass\">",
									include.COMSYS.$$$Text(m$, 414, null,
											SPRACHE)), "</span></div></td>"),
					"\n");
			// << if
			// (+$$$WWWClientParamCoreChangesALL($get(^WWWClientParam(YM,YM,1))))
			...
			if (mOp.Logical((mOp.Positive(include.WWWConst
					.$$$WWWClientParamCoreChangesALL(
							...
							m$.Fnc.$get(m$.var("^WWWClientParam", YM.get(),
									YM.get(), 1))))))) {
				// << write
				// "                            <td width=""202""><font face=Arial><b>&nbsp; <input name="_""""_"YBED"_""""_" type=text class=""estilom3"" size=18 maxlength=30></b></font></td>",!
				m$.Cmd.Write(
						mOp.Concat(
								mOp.Concat(
										mOp.Concat(
												mOp.Concat(
														"                            <td width=\"202\"><font face=Arial><b>&nbsp; <input name=",
														"\""), "YBED"), "\""),
								" type=text class=\"estilom3\" size=18 maxlength=30></b></font></td>"),
						"\n");
			}
			// << }
			// << else {
				...
				// << write
				// "                            <td width=""202""><font face=Arial><b>&nbsp; <input name="_""""_"YBED"_""""_" type=text class=""estilom3"" size=18 maxlength=20></b></font></td>",!
				m$.Cmd.Write(
						mOp.Concat(
								mOp.Concat(
										mOp.Concat(
												mOp.Concat(
														"                            <td width=\"202\"><font face=Arial><b>&nbsp; <input name=",
														"\""), "YBED"), "\""),
								" type=text class=\"estilom3\" size=18 maxlength=20></b></font></td>"),
						"\n");
			}
			// << }
			// << write "                          </tr>",!
			...
			m$.Cmd.Write("                          <tr>", "\n");
			// << write
			// "                            <td height=""22""><div align=""right""><span class=""userandpass"">"_$$$Text("3",,SPRACHE)_"</span></div></td>",!
			// ;Password
			m$.Cmd.Write(
					mOp.Concat(
							mOp.Concat(
									"                            <td height=\"22\"><div align=\"right\"><span class=\"userandpass\">",
									include.COMSYS.$$$Text(m$, "3", null,
											SPRACHE)), "</span></div></td>"),
					"\n");
			// << write
			// "                            <td><font face=Arial><b>&nbsp; <input name=YPWD type=password class=""estilom3"" size=18 maxlength=""20"" maxlenth=20></b></font></td>",!
			m$.Cmd.Write(
					"                            <td><font face=Arial><b>&nbsp; <input name=YPWD type=password class=\"estilom3\" size=18 maxlength=\"20\" maxlenth=20></b></font></td>",
					"\n");
			// << write "                          </tr>",!
					...
					mOp.Concat(
							mOp.Concat(
									...
									include.COMSYS.$$$Text(m$, "67", null,
											SPRACHE)), "&nbsp;\">"), "\n");
			// << write
			// "                                <input type=""reset"" name=""New"" value=""&nbsp;"_$$$Text("58",,SPRACHE)_"&nbsp;"">",!
			// ;New
			m$.Cmd.Write(
					mOp.Concat(
							mOp.Concat(
									"                                <input type=\"reset\" name=\"New\" value=\"&nbsp;",
									include.COMSYS.$$$Text(m$, "58", null,
											SPRACHE)), "&nbsp;\">"), "\n");
			// << write "                              </div>",!
			m$.Cmd.Write("                              </div>", "\n");
		...
		// <<
		// << ;write
		// "<br><br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Sistema de Materiais em Manutenção. O sistema voltará a funcionar normalmente no dia 01/02/2008 (Sexta-feira), à partir das 10:30!",!
		// << write "</body>",!
		m$.Cmd.Write("</body>", "\n");
		// << write "</html>",!
		m$.Cmd.Write("</html>", "\n");
