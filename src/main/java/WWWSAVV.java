...
public class WWWSAVV extends mClass {
  ...
  public void _WWWSAVV() {
    ...
    //<< ;YALLKEY=1 = NUR �FFNEN ;only open
    //<< ;YALLKEY=2 = KEYS VORHANDEN ;on hand
    //<< ;YALLKEY=9 = KEY VOLLST�NDIG KEIN WEITERSUCHEN N�TIG ;KEY integral no necessary
    //<< ;+++++++++++++++++++++++++++++++++++++++
    //<< 
    //<< SET YALLKEY=0             ;SPEICHERN ;Save
    mVar YALLKEY = m$.var("YALLKEY");
    YALLKEY.set(0);
    //<< IF YOPEN=1 SET YALLKEY=1  ;�FFNEN    ;open
        ...
        if (mOp.Equal(mOp.Positive(include.WWWConst.$$$WWW120SaveServerdata(m$,YVOR)),1)) {
          if (mOp.NotEqual(mOp.Positive(include.WWWConst.$$$WWW120PositioningOfButtonLine(m$,YVOR)),1)) {
            do {
              ...
              YMAXKEY.set("");
              //<< . IF YDATEI'="" SET YMAXKEY=$ORDER(^WWW002(0,YDATEI,""),-1) IF YMAXKEY'="" DO  QUIT:YALLKEY=2   ;PRIMAERSCHL�SSEL
                  ...
                  for (YI.set(1);(mOp.LessOrEqual(YI.get(),YMAXKEY.get()));YI.set(mOp.Add(YI.get(),1))) {
                    ...
                    if (mOp.Equal(YKEY.var(YI.get()).get(),"")) {
                      do {
                        ...
                        //<< . . . . SET YALLKEY=1  ;SUCHEN �FFNEN ;seek unclose
                        YALLKEY.set(1);
                      } while (false);
                    }
              ...
              //<< . SET YI=""
              YI.set("");
              //<< . FOR  SET YI=$ORDER(^WWW122s(0,4," ",YFORM,YI)) QUIT:YI=""  DO   ;pr�fen der manuellen felder ;sift who
              for (;true;) {
                ...
                do {
                  ...
                  //<< . . SET $PIECE(YMFELD,Y,YI)=$$GetInternal^WWWTR(YTYP,$PIECE(YMFELD,Y,YI))  ;TYBD;23827;25,06,2003;UMSETZUNG F�R SEITENWECHSEL
        ...
        if (mOp.NotEqual(m$.var("YDATEI").get(),"")) {
          ...
          do {
            ...
            if (mOp.Equal(YTYP.get(),8)) {
              //<< . . . . IF $DATA(^WWW122D(0,YFORM)) DO  ;TYBD;18,10,2004;ANZAHL DECIMALSTELLEN W�HRUNG
            ...
            //<< . . . ;
            //<< . . . IF YTYP=8 IF $FIND($PIECE(YFELD,Y,YI),"@") DO  QUIT  ;FIS;25727;21.05.04;FREMDW�HRUNG
            if (mOp.Equal(YTYP.get(),8)) {
              if (mOp.Logical(m$.Fnc.$find(m$.Fnc.$piece(YFELD.get(),m$.var("Y").get(),YI.get()),"@"))) {
                ...
                //<< . . . . ;
                //<< . . . . IF YDECIMAL="," SET $PIECE(YFELD,Y,YI)=$TRANSLATE($PIECE(YFELD,Y,YI),".",$EXTRACT(TRENN,2))  ;ZUR�CKSETZEN AUF DEZIMAL-TRENNZEICHEN ;upon
      ...
      do {
        ...
        if (mOp.Equal(YMAXKEY.get(),"")) {
          do {
            ...
            //<< . . QUIT:YOPEN=1                                 ;NUR DATENSATZ SUCHEN/�FFNEN ;only data record
            if (mOp.Equal(m$.var("YOPEN").get(),1)) {
              break;
            }
            ...
            if (mOp.Equal(m$.Fnc.$piece(m$.Fnc.$get(m$.var("^WWW122",0,m$.var("YFORM").get(),YI.get(),1)),m$.var("Y").get(),1),"")) {
              ...
              if (mOp.NotEqual(YA.get(),"")) {
                do {
                  ...
                  //<< . . . . IF YOPEN'=2 QUIT:$EXTRACT(YOPEN,1,4)'="SAVE"       ;ZWISCHENSPEICHER F�R MANUELLE ;to
